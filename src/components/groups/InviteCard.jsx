...
function InviteCard({ invite, onAccept, onDecline }) {
  return (
    <TableRow sx={{ height: 50 }}>
      <TableCell
        sx={{ maxWidth: 150, overflow: "hidden", position: "relative" }}
      >
        <Box
          sx={{
            width: "100%",
      ...
      <TableCell align="right" sx={{ width: 60, minWidth: 60 }}>
        <IconButton
          size="small"
          color="success"
          onClick={() => onAccept(invite.group._id, invite._id)}
        >
          <CheckIcon />
        </IconButton>
        <IconButton
          size="small"
          color="error"
          onClick={() => onDecline(invite.group._id, invite._id)}
        >
          <CloseIcon />
        </IconButton>
      </TableCell>
